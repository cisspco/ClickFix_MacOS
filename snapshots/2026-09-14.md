# MacSync Stealer (ClickFix macOS) Snapshot — 2026-09-14 UTC

## 수집 상태
- 지정 출처 접근성: Microsoft — fetched, ThreatFox — blocked, Beelzebub — blocked
- 추가 페치 성공: 없음
- 추가 페치 차단: blog.rankiteo.com, daylight.ai, netskope.com, hexnode.com, cloaked.com
- seed: 7 / 검증: 37 / 미검증: 2
...
이번 실행 검색에서 새로 발견된 후보 리포트 3건 — blog.rankiteo.com(2026-09, "Google/Cloudflare/Zoom/Claude AI/Docker/TradingView/Coinbase/Notion" 관련 MacSync 기사), daylight.ai("MacSync Stealer Returns: SEO Poisoning and Fake GitHub Repositories" — 새로운 배포 경로 시사), netskope.com(MacSync 변종을 언급하는 AppleScript 계열 기사) — 및 이전 실행부터 반복 확인된 hexnode.com, cloaked.com 기사 모두 프록시에 의해 원문 접근이 차단되어 내용을 검증하지 못했다. 특히 daylight.ai 제목은 기존 검색광고(Google Ads) 유인 외에 SEO 포이즈닝·가짜 GitHub 저장소라는 새로운 배포 경로 가능성을 시사하나, 원문 미확인으로 구체적 도메인·해시는 확인되지 않았고 IOC로 채택하지 않는다.

이전 실행에서 확인했던 Infosecurity Magazine/CIS/Red Piranha/GBHackers/Huntress/RST Cloud 등도 이번 실행에서 재시도하지 않았으며(신규 페치 예산은 이번 실행에서 발견한 후보 우선 사용), 기존 미검증 정황(MaaS 운영, Loader-as-a-Service·API 키 C2 전환, 서명된 변종)은 변동 없이 유지된다.

**⚠️ (미검증, 이전 실행에서 이월) 추가 단서:** 검색 스니펫(RST Cloud, 원문 접근 차단)에 따르면 API 키 `5190ef1733183a0dc63fb623357f56d6`가 신규 C2 도메인 후보 **houstongaragedoorinstallers[.]com**과 함께 언급되며, 동일 API 키가 여러 배포본에서 반복 사용되어 동일 MaaS 위협 행위자가 C2 도메인을 순환시키고 있음을 시사한다고 한다. 이번 실행에서도 rstcloud.com 원문에 재접근하지 못해 검증(verified)으로 승격하지 못함.

**⚠️ (미검증) 캠페인 진화 정황 (검색 스니펫만, 원문 미확인):** 다수의 검색 스니펫에 따르면 MacSync는 MaaS(Malware-as-a-Service)로 운영되며, 2026년 2월경 시작된 세 번째 캠페인 웨이브에서 기존 네이티브 Mach-O 배포 방식 대신 셸 기반 로더(Loader-as-a-Service), API 키 기반 C2, 동적 AppleScript 페이로드, 인메모리 실행 방식으로 전환했다는 정황이 있다. 이번 실행에서 새로 발견된 daylight.ai 제목("SEO Poisoning and Fake GitHub Repositories")은 배포 경로가 검색광고 외에 SEO 포이즈닝·가짜 GitHub 저장소로도 확장되었을 가능성을 추가로 시사한다. 원문 미접근으로 구체적 신규 도메인/해시는 확인되지 않았다.

**참고(무관 캠페인, IOC 미채택):** "InstallFix"(Amatera Stealer, Windows/MSIX)와 Netskope/The Register가 보도한 별도의 macOS ClickFix 캠페인(AppleScript 기반 스틸러/RAT, 가짜 CAPTCHA·브라우저 업데이트 유인)이 이번 실행 검색에서도 확인되었다. 둘 다 가짜 Claude 설치 페이지를 사용하지만 배포되는 악성코드 패밀리가 MacSync(macOS Mach-O)와 다르거나(Amatera는 Windows), 캠페인 자체가 별개(Netskope AppleScript 스틸러)로 판단되어 IOC로 채택하지 않는다. 다만 이번 실행에서 확인한 Netskope 관련 검색 스니펫 중 하나가 "MacSync"라는 이름을 직접 언급하여 두 계열이 수렴하고 있을 가능성이 있으나, 원문 접근이 차단되어 검증하지 못했다.
...
## 파일 해시 (0)
### SHA-256
이번 실행에서도 본 캠페인에 연결된 검증 해시 없음. Microsoft(2026-08-18) 보고서는 해시를 포함하지 않았고, ThreatFox·Beelzebub·blog.rankiteo.com·daylight.ai·netskope.com·hexnode.com·cloaked.com은 모두 접근 차단됨.
...
- 신규 보고서: 발견은 했으나 전부 차단 — blog.rankiteo.com, daylight.ai, netskope.com, hexnode.com, cloaked.com (5건, 원문 미확인)
...
- [ThreatFox IOC 1893641](https://threatfox.abuse.ch/ioc/1893641/) — 알 수 없음 — blocked
- [MacSync Stealer: Fake Claude Code Google Ads](https://beelzebub.ai/blog/macsync-stealer-fake-claude-code-google-ads/) — 알 수 없음 — blocked
- [Google, Cloudflare, Zoom, Claude AI... MacSync macOS Stealer](https://blog.rankiteo.com/gooantzoonotdoccoitraclo1789028877-google-cloudflare-zoom-claude-ai-docker-tradingview-coinbase-notion-cyber-attack-september-2026/) — 2026-09 — blocked
- [MacSync Stealer Returns: SEO Poisoning and Fake GitHub Repositories](https://daylight.ai/blog/macsync-stealer-returns-seo-poisoning) — 알 수 없음 — blocked
- [macOS ClickFix Lures Deploy AppleScript Stealer & Persistent RAT](https://www.netskope.com/blog/macos-clickfix-lures-deploy-applescript-stealer-persistent-rat) — 알 수 없음 — blocked
- [ClickFix macOS Malware: Crypto Drainer Targets Keychain Data](https://www.hexnode.com/blogs/clickfix-macos-malware-steals-keychain-data-drains-crypto/) — 알 수 없음 — blocked
- [Cloaked — terminal fix warning](https://www.cloaked.com/post/would-you-paste-this-quick-terminal-fix-on-your-mac--then-watch-your-keychain-and-crypto-leak) — 알 수 없음 — blocked
- FSEC(금융보안원) 2026-09 보안 권고 — seed(advisory)
