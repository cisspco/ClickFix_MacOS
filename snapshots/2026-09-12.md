# MacSync Stealer (ClickFix macOS) Snapshot — 2026-09-12 UTC

## 수집 상태
- 지정 출처 접근성: Microsoft — fetched, ThreatFox — blocked, Beelzebub — blocked
- 추가 페치 성공: [Unit42 timely-threat-intel GitHub 문서](https://github.com/PaloAltoNetworks/Unit42-timely-threat-intel/blob/main/2026-06-20-ClickFix-campaign-delivers-macOS-infostealer-via-DMG.txt) (단, 본 캠페인과 무관 — 아래 참고 참조, IOC 미채택)
- 추가 페치 차단: rstcloud.com, daylight.ai, blog.rankiteo.com, labs.cloudsecurityalliance.org
- seed: 7 / 검증: 37 / 미검증: 2
...
**⚠️ (미검증) 캠페인 진화 정황 (검색 스니펫만, 원문 미확인):** 다수의 검색 스니펫(gbhackers.com, redpiranha.net, cisecurity.org 등, 모두 원문 차단)에 따르면 MacSync는 MaaS(Malware-as-a-Service)로 운영되며, 2026년 2월경 시작된 세 번째 캠페인 웨이브에서 기존 네이티브 Mach-O 배포 방식 대신 셸 기반 로더(Loader-as-a-Service), API 키 기반 C2, 동적 AppleScript 페이로드, 인메모리 실행 방식으로 전환했다는 정황이 있다. 원문 미접근으로 이번 실행에서는 검증하지 못했으며, 구체적 신규 도메인/해시는 확인되지 않았다.

**참고(무관 캠페인, IOC 미채택):** 이번 실행에서 Unit42 GitHub 문서(2026-06-20, ClickFix macOS infostealer via DMG)를 성공적으로 페치했으나, 본문에 MacSync·Claude·Anthropic에 대한 언급이 전혀 없어 본 트래커가 추적하는 캠페인과 무관한 별도의 ClickFix/macOS 인포스틸러 활동으로 판단했다. 해당 문서가 제시한 도메인(svs-verificationdate[.]beer, fewfwfwfwfwf[.]info), IP(178.16.52[.]101, 196.251.107[.]171), 해시 4건은 본 캠페인과의 연결고리가 없어 블록리스트·해시 목록에 채택하지 않는다.

**참고(비관련/별도 캠페인, IOC 미채택, 이전 실행과 동일):** "InstallFix"로 명명된 별도 캠페인(Malwarebytes, Rapid7, Bitdefender, Trend Micro, PushSecurity, SOC Prime 등 다수 보도, 이번 실행에서도 재확인)은 가짜 Claude Code/Claude AI 설치 페이지와 Google 광고를 사용하지만 배포 악성코드가 **Amatera Stealer**(Windows, MSIX 기반)로 본 트래커가 추적하는 MacSync(macOS, Mach-O) 계열과는 다른 패밀리다. 따라서 IOC로 채택하지 않는다. Netskope/The Register가 보도한 별도의 macOS ClickFix 캠페인(AppleScript 기반 스틸러/RAT, 가짜 CAPTCHA·브라우저 업데이트 유인)도 가짜 Claude 설치 페이지 체인과 무관하여 채택하지 않는다.
...
## 파일 해시 (0)
### SHA-256
이번 실행에서도 본 캠페인에 연결된 검증 해시 없음. Microsoft(2026-08-18) 보고서는 해시를 포함하지 않았고, ThreatFox·Beelzebub·RSTCloud·Daylight·Rankiteo·CSA는 모두 접근 차단됨. Unit42 GitHub 문서는 해시 4건을 포함하나 본 캠페인과 무관하여 채택하지 않음 (위 참고 참조).
...
- 신규 해시: 없음 (Unit42 문서의 해시 4건은 무관 캠페인으로 판단되어 미채택)
- 승격(미검증→검증): 없음
- 폐기(sinkhole/takedown): 없음
- 신규 보고서: 없음 (Microsoft 2026-08-18 재확인, Unit42 2026-06-20 문서는 페치했으나 무관 캠페인으로 확인됨; RSTCloud/Daylight/Rankiteo/CSA는 모두 접근 차단)
...
- [MacSync Stealer Returns: SEO Poisoning and Fake GitHub Repositories](https://daylight.ai/blog/macsync-stealer-returns-seo-poisoning) — 날짜 미상 — blocked
...
- [Unit42 timely-threat-intel — ClickFix campaign delivers macOS infostealer via DMG](https://github.com/PaloAltoNetworks/Unit42-timely-threat-intel/blob/main/2026-06-20-ClickFix-campaign-delivers-macOS-infostealer-via-DMG.txt) — 2026-06-20 — fetched (본 캠페인과 무관, IOC 미채택)
